...
";
            ...
            // GeoIP Country Blocker / Whitelister Directive (Uses Cloudflare CF-IPCountry header)
            $geoIpBlock = "";
            $geoMode = $d['geoip_mode'] ?? 'off';
            $geoCountries = strtoupper(trim($d['geoip_countries'] ?? ''));
            if ($geoMode !== 'off' && !empty($geoCountries)) {
                $countryList = array_map('trim', explode(',', $geoCountries));
                $countryList = array_filter($countryList);
                if (!empty($countryList)) {
                    $countryRegex = implode('|', $countryList);
                    if ($geoMode === 'allow_only') {
                        // Allow ONLY listed countries (Block others)
                        $geoIpBlock = "
    # GeoIP Protection: Allow ONLY {$geoCountries}
    if (\$http_cf_ipcountry !~* ^({$countryRegex})$) {
        return 403 \"403 Forbidden: Access not allowed from your country (\$http_cf_ipcountry)\";
    }
";
                    } elseif ($geoMode === 'block_only') {
                        // Block listed countries
                        $geoIpBlock = "
    # GeoIP Protection: Block {$geoCountries}
    if (\$http_cf_ipcountry ~* ^({$countryRegex})$) {
        return 403 \"403 Forbidden: Access blocked from your country (\$http_cf_ipcountry)\";
    }
";
                    }
                }
